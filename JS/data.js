	...
	fetch("data/data.json").then((response) =>
		...
		for (let k in myObject) {
			if (myObject[k].filter_id === filter_id) {
				...
				document.querySelectorAll(".myRow")[0].style.width = "auto"
				...
				for (let m in myImages) {
					...
					if (!(String(myImages[m].innerHTML) === "undefined") && (typeof (myImages[m]) === "object")) {
						...
						myImages[m].style.width = "280px"
						myImages[m].style.height = "230px"
						myImages[m].style.objectFit = "contain" //added//
						myImages[m].style.backgroundColor = "#ffffff"
						myImages[m].style.margin = "10px"
				...
				for (let j in myCartButton) {
					if (!(String(myCartButton[j].innerHTML) === "undefined") && (typeof (myCartButton[j]) === "object")) {
						...
						myCartButton[j].style.margin = "10px 10px 10px auto"//added//
						myCartButton[j].style.marginTop = "5px"
						myCartButton[j].style.color = "white"
						myCartButton[j].style.backgroundColor = "black"
...
function displayProduct(product) {
	...
	return `<div id="p${product.product_id}">
		...
		<br>
	<HR style="margin:2px 0;background-color:grey;">
...
function addToCart(product_id, product_name, imgSrc, price) {
	...
	if (!(cookieValue === "") && !(cookieValue === null)) {
		...
		if (!(uname === null) && !(uname === "")) {
			...
			let newImgArr = new Array()
			newImgArr.push(imgSrc)
			localStorage.setItem("imgSrcs", newImgArr)

			let newPriceArr = new Array()
			newPriceArr.push(price)
			localStorage.setItem("prices", newPriceArr)
...
function checkOut() {
	...
	for (let i = 0; i < myImageArr.length; i++) {
		...
		grandTotal += tot
		myTable += `<tr><td class="table-product-content"><img src="${myImageArr[i]}" style="width:100px;height:100px;object-fit:contain;border:2px solid black;"></td><td class="table-product-items">${qtyArr[i]}</td><td class="table-product-items">₹${priceArr[i]}</td><td class="table-product-total">₹${tot}</td></tr>`
...
function proceedPayment(total) {
	let cardDiv = `<img src="images/visa.jpg"><img src="images/mastercard.jpg"><img src="images/amex.png"><img src="images/discover.png">`
	...
	cardDiv += `<div style="margin:10px 10px auto auto;float:right;">CVV/CVV2:<input type="password" style="appearance:none;padding-left:15px;width:70px;background-image:url('images/lock-fill.jpeg');background-size:15px auto;background-position:1px 5px;background-repeat:no-repeat;" maxlength="3"></div></div>`
...
function paymentSuccessfull() {
	alert("Thank you for shopping with us❤️. Your order will be delivered shortly!!")
	clearCart()
	document.getElementById("finalBill").innerHTML = ""
	document.getElementById("finalBill").style.display = "none"
}